...
export default Card = ({ restID, restaurant }) => {
  const imageURL =
    `https://res.cloudinary.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_508,h_320,c_fill/` +
    restaurant.data.cloudinaryImageId;
  const navigate = useNavigate();
  const clickHandler = () => navigate("/restaurant/" + restID);
  useEffect(() => {
    const restaurantCard = document.getElementById(restID);

    restaurantCard.addEventListener("click", clickHandler);
    return () => {
      restaurantCard.removeEventListener("click", clickHandler);
    };
  }, []);

  return (
    <div className="card" id={restID}>
      <div>
          ...
          <div
            ...
            style={
              restaurant.data.avgRating == "--"
                ? { backgroundColor: "white", color: "#535665" }
                : restaurant.data.avgRating >= 4
                ? { backgroundColor: "#48c479" }
                : restaurant.data.avgRating >= 2
